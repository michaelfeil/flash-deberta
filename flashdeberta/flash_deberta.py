...
class FlashDisentangledSelfAttention(nn.Module):
    """
    ...
    def forward(
        ...
        """
        def kernel(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, rel_embeddings: Optional[torch.Tensor], attn_mask: Optional[torch.Tensor], scale: float = 1.0, ):
            self = None
            scale = torch.sqrt(torch.tensor(q.size(-1), dtype=torch.float) * scale_factor)
            attention_scores = torch.bmm(q, k.transpose(-1, -2) / scale.to(dtype=q.dtype))
            if rel_embeddings is not None:
                # rel_embeddings = self.pos_dropout(rel_embeddings) # TODO: backward uses dropout
